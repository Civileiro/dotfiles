...
in
{
  ...
  config = mkIf (tCfg.active == "catppuccin") (mkMerge [
    ...
    (mkIf config.services.displayManager.sddm.enable {
      ...
      services.displayManager.sddm.theme = "catppuccin-${cfg.flavour}-${cfg.accent}";
      environment.systemPackages = with pkgs; [
    ...
    (mkIf config.modules.desktop.terminal.alacritty.enable {
      modules.desktop.terminal.alacritty.configImports =
        let
        ...
        in
        [ (catppuccin-alacritty + "/catppuccin-${cfg.flavour}.toml") ];
    })
    (mkIf config.modules.desktop.terminal.kitty.enable {
      modules.desktop.terminal.kitty.configImports =
        ...
        in
        [ (catppuccin-kitty + "/themes/${cfg.flavour}.conf") ];
    })
  ]);
}
